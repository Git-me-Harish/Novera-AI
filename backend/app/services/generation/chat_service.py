...
class ChatService:
    ...
    async def chat(
        ...
        if stream:
            ...
            # _chat_stream is an async generator (it uses `yield`).
            # We must NOT await it — just return the generator object.
            # The StreamingResponse in the endpoint iterates it with `async for`.
        ...
        # ── 10. Suggestions — fire as background task, never block response ─────
        # The docstring always said "background task" but the code called a sync
        # private method inline, blocking for up to several hundred ms.
        # We now schedule it properly and return immediately with fallback
        # suggestions. The frontend can optionally poll for richer suggestions
        # but the main answer is never delayed.
        ...
        # Fire the richer LLM-based generation in the background so it can
        # warm the cache for the next request without delaying this response.
        try:
